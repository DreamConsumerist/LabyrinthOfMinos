...
7. **(Added 2026-08-02, post-6.5-migration)** Migrate off the deprecated `com.unity.services.relay` package to the unified `com.unity.services.multiplayer` package (Lobby + Matchmaker + Relay under one API). Not urgent — the deprecated package still functions — but do it as its own isolated pass, after `RelayManager.cs`'s post-6.5 `RelayServerData` rewrite has been verified against a live host+join session. Bundling the two Relay-touching changes together would make a regression hard to attribute.
